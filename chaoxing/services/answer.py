...
from chaoxing.utils.answer_check import *
...
class Tiku:
    ...
    def query_with_meta(self, q_info: dict) -> AnswerQueryResult:
        ...
        if answer:
            ...
            if check_answer(normalized_answer, question['type'], self):
                ...
                return AnswerQueryResult(
                    ...
                    source=self._infer_answer_source(),
                    confidence=self._estimate_confidence(normalized_answer),
                    candidates=self._build_candidates(normalized_answer),
                    raw_response=self._build_raw_response(normalized_answer),
                    provider=self.name or "",
                    matched=True,
                )
        ...
        logger.error(f"从{self.name}获取答案失败：{question['title']}")
        return AnswerQueryResult(
            answer=None,
            source=self._infer_answer_source(),
            confidence=0.0,
    ...
    def _estimate_confidence(self, answer: Any) -> float:
        if answer is None:
            return 0.0
        return 0.85 if self._infer_answer_source() == "ai" else 0.95

    def _build_candidates(self, answer: Any) -> list[str]:
        if answer is None:
            return []
    ...
    def _build_raw_response(self, answer: Any) -> Any:
        if answer is None:
            return None
